...
@app.route('/')
def form():
    years = [str(y) for y in range(2016,2027)][::-1]
...
def classes():
    ...
    start_month = locale().month_number(request.form['start-month'].lower())
    start_day = int(request.form['start-day'])
    last_month = locale().month_number(request.form['last-month'].lower())
    last_day = int(request.form['last-day'])
    weekdays = request.form.getlist('days')
    date_fmt = [b for (a, b) in date_formats() if a == request.form['format']][0]
    print(start_day, start_month, "here")
...
#building out route for Carleton
@app.route('/results_carleton/', methods=['POST'])
def results_carleton():
    year = int(request.form['year'])
    start_month = locale().month_number(request.form['start-month'].lower())
    start_day = int(request.form['start-day'])
    last_month = locale().month_number(request.form['last-month'].lower())
